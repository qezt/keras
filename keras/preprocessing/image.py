...
def load_img(path, grayscale=False, target_size=None, resampler_name='lanczos'):
    ...
    # Arguments
        ...
        resample: one of 'lanczos', 'nearest', 'bilinear', 'bicubic', 'box', 'hamming'
    ...
    if pil_image is None:
        raise ImportError('Could not import PIL.Image. '
                          'The use of `array_to_img` requires PIL.')
    resampler_name = resampler_name.upper()
    resamplers = ['BICUBIC', 'BILINEAR', 'BOX', 'HAMMING', 'LANCZOS', 'NEAREST']
    if resampler_name not in resamplers:
        raise ValueError('resampler_name parameter should be one of %s' % (', '.join((name.lower() for name in resamplers))))
    resample_value = pil_image.__dict__[resampler_name]
